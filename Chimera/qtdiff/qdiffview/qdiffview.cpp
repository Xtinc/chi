...
#include <QMap>
#include "diff.h"
#include "qtdiff/dtl/dtl.hpp"

QString bodyTemplate1 = "<html>\n"
                        "<head>\n"
                        "<style>\n"
                        ".text {font-weight:600;}\n"
                        "body {background-color: #19232D;width: 100%;margin: 0;}\n"
                        "table {width: 100%; }\n"
                        "th {color: #b2b2b2;}\n"
                        ".rp {background-color: red;}\n"
                        ...
                        "</head>\n"
                        "<body>\n"
                        "<table width=\"100%\" cellspacing=\"1\" cellpadding=\"3\" style=font-family:\"consolas\">\n";
...
QString commonTemplate2
    = "<tr class=\"cl2\"><th align=\"right\" width=\"40\">%1</th><th align=\"right\" "
      "width=\"40\">%2</th><td width=\"100%\">%3</td></tr>\n";
QString commonLinkTemplate = "<tr class=\"cl2\"><th width=\"40\" colspan=\"2\"><font color=#19232D>&gt;&#x2195;&lt;</font></th><td  "
...
void QDiffView::_update()
{
    ...
    for (int i = 0; i < this->_diffs.length(); i++) {
        ...
        if (diff->type == dtl::SES_COMMON) {
            ...
            if (length > 7 && !diff->visible) {
                ...
                contents.append(commonLinkTemplate);
    ...
    if(contents.length()>1000){
        QFile htmlfile("diffres.html");
        htmlfile.open(QFile::WriteOnly);
        QTextStream out(&htmlfile);
        out<<contents.join("");
        htmlfile.close();
        this->load(QUrl::fromUserInput(htmlfile.fileName()));
    }
    this->setHtml(contents.join(""));*/
    ...
    out<<contents.join("");
    htmlfile.close();
    QDesktopServices::openUrl(QUrl::fromUserInput(QDir::currentPath()+QDir::separator()+htmlfile.fileName()));
}
