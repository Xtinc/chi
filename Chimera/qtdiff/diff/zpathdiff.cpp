...
void ZPathDiff::setFilters(const QString &fliter)
{
    if(fliter.isEmpty()){
        return;
    }
    else{
        list=fliter.split(",",QString::SkipEmptyParts);
    }
}

QStringList ZPathDiff::GetFilters() const
{
    return list;
}
...
QList<ZPathDiffModel> ZPathDiff::execute()
{
    ...
    QFileInfoList srcFileInfoList = ZFile::files(mPathSrc,list);
    QFileInfoList dstFileInfoList = ZFile::files(mPathDst,list);
