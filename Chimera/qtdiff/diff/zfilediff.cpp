...
QList<ZFileDiffModel> ZFileDiff::execute()
{
    ...
    {
        using namespace dtl;
        Diff<unsigned int> D(mHashSrcLst, mHashDstLst);
        D.compose();
        std::vector<std::pair<unsigned int,elemInfo>> consequence = D.getSes().getSequence();
        for ( auto i :consequence)
        {
            ...
            case SES_ADD:
                model.setSrcHash(0);
                model.setDstHash(i.second.afterIdx-1);
                model.setStatus(Added);
                break;
            case SES_DELETE:
                model.setSrcHash(i.second.beforeIdx-1);
                model.setDstHash(0);
